 ...
 * Iolaus: the hook also receives the planned matches and may return a reason string to
 * veto the mutation pass (used for host permission checks on the files to be written).
 */
export interface RewriteHooks {
  readonly onPreviewComplete?: (matches: readonly RewriteMatch[]) => void | string | Promise<void | string>;
}
...
export async function executeRewrite(
  ...
  const vetoed = await hooks?.onPreviewComplete?.(matches);
  if (typeof vetoed === "string") {
    return failure("PERMISSION_DENIED", vetoed, "apply", input.language, elapsed(), { stderr: preview.stderr });
  }

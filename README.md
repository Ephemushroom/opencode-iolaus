...
Iolaus keeps agent state in two layers. `~/.iolaus` (or `IOLAUS_HOME`) is shared across projects: `models.json`, `verify.json`, and `agent/plans`, `agent/skills`. `<project>/.iolaus` holds DAG state and project plans and overrides the shared files. Both are created on first run; existing files are never touched. Put a skill at `~/.iolaus/agent/skills/<name>/SKILL.md` (or `.iolaus/skills/<name>/SKILL.md` for one project) and every agent can invoke it as `iolaus-home:<name>`. Iolaus does not keep its own memory; use your memory plugin for that.

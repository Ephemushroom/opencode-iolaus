...
Iolaus keeps agent state in two layers. `~/.iolaus` (or `IOLAUS_HOME`) is shared across projects: `models.json`, `verify.json`, and `agent/plans`, `agent/memory`, `agent/skills`. `<project>/.iolaus` holds DAG state and project plans and overrides the shared files. Both are created on first run; existing files are never touched. Put a skill at `~/.iolaus/agent/skills/<name>/SKILL.md` (or `.iolaus/skills/<name>/SKILL.md` for one project) and every agent can invoke it as `iolaus-home:<name>`. Agents keep durable notes through Code Mode `tools.memory.list/read/write/remove`; the four primary agents and Metis may write, read-only specialists may only read. Notes are plain markdown files you can edit yourself.

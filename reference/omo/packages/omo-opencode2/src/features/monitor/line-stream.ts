...
const ANSI_PATTERN = new RegExp(
  ["\\x1b\\[[0-?]*[ -/]*[@-~]", "\\x1b\\][^\\x07]*(?:\\x07|\\x1b\\\\)", "\\x1b[@-_]"].join("|"),
  "g",
)
...
function trimTrailingCarriageReturn(bytes: Uint8Array): Uint8Array {
  if (bytes.length > 0 && bytes[bytes.length - 1] === 0x0d) return bytes.slice(0, bytes.length - 1)
  return bytes
}

/**
 * Turns raw process bytes into lines: strips ANSI, caps per-line length, and
 * suppresses binary chunks rather than pushing control bytes at the model.
 */
export class LineStream {
  ...
  feed(chunk: Uint8Array): LineStreamResult {
    if (chunk.length === 0) return { lines: [], binarySuppressedBytes: 0 }
    if (this.isBinaryChunk(chunk)) {
      return { lines: [{ text: "", rawText: "", binary: true }], binarySuppressedBytes: chunk.byteLength }
    }
    return this.consumeChunk(chunk)
  }

  flush(): LineStreamResult {
    if (this.pending.length === 0) return { lines: [], binarySuppressedBytes: 0 }
  ...
  private consumeBytes(bytes: Uint8Array, lines: DecodedLine[]): void {
    if (this.truncating || bytes.length === 0) return
  ...
  private emitPendingLine(lines: DecodedLine[]): void {
    if (!this.truncating) lines.push(this.createLine(this.pending, false))
    this.pending = new Uint8Array()
    this.truncating = false
  }
  ...
  private createLine(bytes: Uint8Array, truncated: boolean): DecodedLine {
    ...
    const line: DecodedLine = { text: stripAnsi(rawText), rawText }

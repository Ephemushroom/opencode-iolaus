...
/**
 * Rejects nested unbounded quantifiers such as `(a+)+`, which can hang the
 * process on a hostile line. Monitor patterns run against every line of a hot
 * output stream, so a catastrophic pattern is a denial of service on the agent.
 */
function isPotentiallyCatastrophicRegex(pattern: string): boolean {
  ...
  for (let i = 0; i < pattern.length; i += 1) {
    ...
    if (ch === ")") {
      ...
      if (parent && (quant.unbounded || frame?.hasUnboundedQuant)) parent.hasUnboundedQuant = true
      i += quant.length
      continue
    }
    if (ch === "*" || ch === "+" || ch === "{") {
      ...
      const parent = groupStack[groupStack.length - 1]
      if (quant.unbounded && parent) parent.hasUnboundedQuant = true
...
export function createMonitorFilter(
  ...
  if (pattern.length > opts.patternMaxLength) {
    return { filter: null, error: "pattern too long" }
  }
  ...
  return { filter: { matches: (text: string) => re.test(stripAnsiColors(text)) }, pattern }
}

...
/**
 * Wraps a batch for injection into the session.
 *
 * The untrusted-observation banner is load-bearing and must not be dropped:
 * this is arbitrary process output being placed into the model's context, and
 * the model has to be told not to treat it as instructions.
 */
...
function formatStatus(record: EnvelopeRecord, batch: OutputBatch): string {
